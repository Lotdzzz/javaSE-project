...
JVM 到底在背后做了什么？为什么调用proxy.rentHouse()，就会走到invoke()方法里？

JVM 的底层执行流程（运行时，4 步完成动态代理）

当你执行 Proxy.newProxyInstance() 时，JVM 在运行期偷偷做了这 4 件事，全程在内存中完成，不会生成任何.class 文件：

 - 通过反射读取信息：JVM 通过传入的类加载器和接口数组，反射读取目标类的接口、方法、参数等元数据；
 - 动态生成字节码：在内存中，自动生成一个代理类的字节码，类名默认是 $Proxy0（开头，数字递增，比如Proxy1、Proxy2）；
 - 代理类的特点：这个自动生成的$Proxy0类，实现了和目标类相同的接口（比如RentService），并且持有InvocationHandler的对象；
 - 创建代理对象：JVM 加载这个字节码，生成代理类的 Class 对象，最后创建代理对象并返回。
...
你调用proxy.rentHouse()，本质是调用了$Proxy0的rentHouse()，而这个方法里，又调用了你写的invoke()方法！所有的增强逻辑，都在invoke()里执行，这就是动态代理的底层逻辑！
